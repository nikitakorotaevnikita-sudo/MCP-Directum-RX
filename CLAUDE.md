...
**Перед началом любой задачи** (фича, фикс, рефакторинг) — поискать в Obsidian vault:

```
C:\Users\Korotaev_NO\Desktop\Obsidian vault\Прототипы\
```

Что искать:
- Тикеты с похожим названием (`Тикеты/TICKET-*.md`)
- Спеки и планы (`SPEC-*.md`, `Планы/`)
- Исследования по теме (`Исследования/`)
- Пересечения с другими агентами (`ИИ Агенты/agent_porucheniya/`, `agent_execution_control/`, etc.)

Как искать: `Grep` по ключевым словам задачи, потом читать найденные файлы.

Зачем: тикеты содержат acceptance criteria, контекст решений, ссылки на смежные проекты.
