...
## Дополнение (2026-09-25): срок «сегодня / неделя» и часовой пояс

**Параметр `due`** (`today` | `week`, по умолчанию нет) — в `list_action_items` (свои поручения, все пользователи)
и в `admin_list_employee_action_items`:

- `today` — срок в пределах сегодняшних суток; `week` — с начала сегодняшних суток по конец дня «сегодня + 6» (7 календарных дней).
- Только поручения в работе; уже просроченные в прошлые дни не попадают.
- Несовместим с `date_from`/`date_to`, `only_overdue` и `status` ∉ {`in_process`, `all`} — ошибка тула.
- В `ActionItemFilters` — поле `due`; в ответе админ-тула `filters.due`.

**Часовой пояс.** RX хранит сроки в местном времени стенда (`…T00:00:00+04:00`). Границы дней (период и `due`)
строятся в поясе стенда, «сегодня» считается в нём же. Настройка `MCP_UTC_OFFSET` (например `+04:00`);
не задана — пояс машины, где запущен сервер. Прежние границы в UTC (`…T00:00:00Z`) теряли граничный день —
проверено на стенде: за 01.08.2023 с UTC-границами 0 поручений, с `+04:00` — 1.
